...
public class PipeItemFixture
{
    ...
    public void Get_contained_value()
    {
        ...
        Assert.That(new PipeItem<int>([42]).GetArray(), Is.EqualTo(new[]{42}));
    }
    ...
    public void Checks_casting()
    {
        ...
        Assert.Throws<InvalidCastException>(() => new PipeItem<int>([42]).GetValue());
    }
    ...
    public void Check_contained_value()
    {
        ...
        Assert.True(new PipeItem<int>([42]).IsArray);
        Assert.False(new PipeItem<int>([42]).IsValue);
        Assert.False(new PipeItem<int>(42).IsEmpty);
    ...
    {
        Assert.That(PipeItem<int>.Empty.Where(_ => false), Is.EqualTo(PipeItem<int>.Empty));
        Assert.That(PipeItem<int>.Empty.Where(_ => true), Is.EqualTo(PipeItem<int>.Empty));
    }
    ...
    public void Filter_with_single_value()
    {
        ...
        Assert.That(item.Where(_ => false), Is.EqualTo(PipeItem<int>.Empty));
    ...
    {
        var item = new PipeItem<int>([42, 100]);
    ...
    public void Equals_for_array()
    {
        ...
        Assert.That(item1, Is.EqualTo(item2));
        Assert.That(new PipeItem<int>(values), Is.Not.EqualTo(new PipeItem<int>([42])));
    }
    ...
    public void Item_size()
    {
        ...
        Assert.That(new PipeItem<int>([42, 100]).Size, Is.EqualTo(2));
    }
}