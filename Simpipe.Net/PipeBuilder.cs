namespace Youscan.Core.Pipes
{
    public class PipeBuilder<T>
    {
        ...
        public DynamicBatchPipeOptions<T> DynamicBatch(Action<T[]> action) => new(PipeAction<T>.For(action));
        public DynamicBatchPipeOptions<T> DynamicBatch(Func<T[], Task> action) => new(PipeAction<T>.For(action));
    }
}